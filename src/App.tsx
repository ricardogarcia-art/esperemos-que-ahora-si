...
import type { BusinessFormData } from './types.ts';
...
import { generateIllustrativeImage, generateLandingPageHtml } from './services/geminiService.ts';
...
const App: React.FC = () => {
  ...
  const handleFormSubmit = useCallback(async (data: BusinessFormData) => {
    ...
    try {
      ...
      // Si el usuario subió imágenes, usa la primera directamente.
      // Si no, genera una nueva imagen usando IA.
      if (data.images && data.images.length > 0) {
        imageUrl = data.images[0];
        setGeneratedImage(imageUrl);
        setIsLoadingImage(false); // La imagen ya está "cargada"
      } else {
      ...
      // Reemplaza el placeholder con la URL de la imagen (ya sea la subida o la generada)
